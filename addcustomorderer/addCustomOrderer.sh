...
export FABRIC_CFG_PATH=${PWD}/${CUSTOM_ORDERER_FOLDER_PATH}
export VERBOSE=false
...
function generateCustomOrderer() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    docker-compose -f $COMPOSE_FILE_CA_CUSTOM_ORDERER up -d 2>&1
    #. fabric-ca/registerEnroll.sh
    . ${CUSTOM_ORDERER_FOLDER_PATH}/fabric-ca/registerEnroll.sh
...
function generateCustomOrdererDefinition() {
  which configtxgen
  if [ "$?" -ne 0 ]; then
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD/${CUSTOM_ORDERER_FOLDER_PATH}
   set -x
   configtxgen -printOrg ${customORG}MSP > ./organizations/ordererOrganizations/$customOrg.beefsupply.com/$customOrg.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
...
function removeCustomOrderer(){ 
  ...
  ${CONTAINER_CLI} cp ./${CUSTOM_ORDERER_FOLDER_PATH}/customscripts cli:/opt/gopath/src/github.com/hyperledger/fabric/peer/customscripts/
 ...
 ./${CUSTOM_ORDERER_FOLDER_PATH}/customscripts/removeCustomNoCLI.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_CUSTOM_ORDERER} -f ${COMPOSE_FILE_CA_CUSTOM_ORDERER} down --volumes -remove-orphans
  ...
  ${CONTAINER_CLI} volume stop peer0.$customOrg.beefsupply.com
  ${CONTAINER_CLI} volume stop docker_$customOrg.beefsupply.com
  
  ${CONTAINER_CLI} volume rm docker_peer0.$customOrg.beefsupply.com
  ${CONTAINER_CLI} volume rm peer0.$customOrg.beefsupply.com
  ${CONTAINER_CLI} volume rm docker_$customOrg.beefsupply.com

  ${CONTAINER_CLI} stop ca_$customOrg
  ${CONTAINER_CLI} rm ca_$customOrg
  ${CONTAINER_CLI} stop $customOrg.beefsupply.com
  ${CONTAINER_CLI} rm $customOrg.beefsupply.com
  ...
  chmod 777 -R $PWD/organizations/ordererOrganizations/${customOrg}.beefsupply.com
  rm -R $PWD/organizations/ordererOrganizations/${customOrg}.beefsupply.com
  ${CONTAINER_CLI} system prune 
  ${CONTAINER_CLI} volume prune 
}

function CustomOrdererUp () {
  # start rancher nodes
...
comment
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    ...
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_CUSTOM_ORDERER} up -d 2>&1
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
...
function addCustomOrderer () {
  ...
  if [ ! -d "./organizations/ordererOrganizations/$customOrg.beefsupply.com" ]; then
    generateCustomOrderer
    generateCustomOrdererDefinition
...
comment
  ...
  ./${CUSTOM_ORDERER_FOLDER_PATH}/customscripts/modifyChannelConfigNoCLI.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi
...
COMPOSE_FILE_CUSTOM_ORDERER=$PWD/${CUSTOM_ORDERER_FOLDER_PATH}/compose/docker/docker-compose-custom.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_CUSTOM_ORDERER=$PWD/${CUSTOM_ORDERER_FOLDER_PATH}/compose/docker/docker-compose-ca-custom.yaml
# default image tag
IMAGETAG="latest"
