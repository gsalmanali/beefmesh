...
createAnchorPeerUpdate() {
  ...
  source ./blockchain/scripts/blockchainEnvVar/$ORG/PeerAnchor.sh
  ...
  #  HOST="peer1.regulator.beefsupply.com"
   ...
   # PORT=11051
  #elif [ $ORG = "${customOrg}0" ]; then
  #  HOST="peer0.${customOrg}.beefsupply.com"
   # PORT=${CUSTOM_CORE_PEER_ADDRESS_PORT}
...
updateAnchorPeer() {
  ...
  peer channel update -f blockchain/channel-artifacts/${CORE_PEER_LOCALMSPID}_update_in_envelope.pb -o $CORE_ORDERER_PEER_ADDRESS --ordererTLSHostnameOverride $CORE_ORDERER_CONTAINER_ADDRESS -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
...
readarray -t lines < $SCRIPT_DIR/../OrgConfigurations/ChannelOrgConfig/$CHANNEL_FOLDER_PATH/joinChannelOrgs.txt
# Display the lines
