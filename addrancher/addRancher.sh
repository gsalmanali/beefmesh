...
export FABRIC_CFG_PATH=${PWD}/addrancher
export VERBOSE=false
...
function generateRancher() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    docker-compose -f $COMPOSE_FILE_CA_RANCHER up -d 2>&1
    #. fabric-ca/registerEnroll.sh
    . addrancher/fabric-ca/registerEnroll.sh
...
function generateRancherDefinition() {
  which configtxgen
  if [ "$?" -ne 0 ]; then
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD/addrancher
   set -x
   configtxgen -printOrg RancherMSP > ./organizations/peerOrganizations/rancher.beefsupply.com/rancher.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
...
function removeRancher(){ 
  ...
  ${CONTAINER_CLI} cp ./addrancher/rancherscripts cli:/opt/gopath/src/github.com/hyperledger/fabric/peer/rancherscripts/
 ...
 ./addrancher/rancherscripts/removeRancherNoCLI.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_RANCHER} -f ${COMPOSE_FILE_COUCH_RANCHER} -f ${COMPOSE_FILE_CA_RANCHER} down --volumes -remove-orphans
  ...
  ${CONTAINER_CLI} volume stop docker_peer0.rancher.beefsupply.com
  ${CONTAINER_CLI} volume rm docker_peer0.rancher.beefsupply.com

  ${CONTAINER_CLI} stop ca_rancher
  ${CONTAINER_CLI} rm ca_rancher
  ${CONTAINER_CLI} stop peer0.rancher.beefsupply.com
  ${CONTAINER_CLI} rm peer0.rancher.beefsupply.com
  ${CONTAINER_CLI} stop couchdb20
  ${CONTAINER_CLI} rm couchdb20
...
function addRancher () {
  ...
  if [ ! -d "./organizations/peerOrganizations/rancher.beefsupply.com" ]; then
    generateRancher
...
comment
  ...
  ./addrancher/rancherscripts/joinChannelNoCLI.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_RANCHER=$PWD/addrancher/compose/docker/docker-compose-rancher.yaml
