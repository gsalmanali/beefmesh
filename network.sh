...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Breeder Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-breeder.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    infoln "Creating Farmer Identities"
    peer_cert peer peer0.farmer.beefsupply.com farmer
    peer_cert admin Admin@farmer.beefsupply.com farmer

    infoln "Creating Breeder Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.breeder.beefsupply.com breeder
    peer_cert admin Admin@breeder.beefsupply.com breeder

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.beefsupply.com
    orderer_cert admin Admin@beefsupply.com
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    ${CONTAINER_CLI_COMPOSE} -f blockchain/compose/docker/$COMPOSE_FILE_CA up -d 2>&1
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/manager/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Manager Identities"
  ...
  infoln "Generating CCP files for Farmer and Breeder"
  ./organizations/ccp-generate.sh
  infoln "Done Generating CCP files for Farmer and Breeder"
}
...
function networkUp() {
  ...
  echo ${COMPOSE_FILES}
  echo ${CONTAINER_CLI_COMPOSE}
  
  #COMPOSE_FILES="-f compose/docker/${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
  COMPOSE_FILES="-f blockchain/compose/docker/${COMPOSE_FILE_BASE}"

  if [ "${DATABASE}" == "couchdb" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f blockchain/compose/docker/${COMPOSE_FILE_COUCH}"
...
function createChannel() {
  ...
  blockchain/scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true
}


function createChannelCustom() {
  # Bring up the network if it is not already up.
  bringUpNetwork="false"
  ...
  # now run the script that creates a channel. This script uses configtxgen once
  # to create the channel creation transaction and the anchor peer updates.
  blockchain/scripts/createChannelCustom.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true
}
...
function deployCC() {
  blockchain/scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
## Call the script to deploy a chaincode to the channel
function deployChaincodeCustom() {
  blockchain/scripts/deployChaincodeCustom.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function deployCarbonCC() {
  blockchain/scripts/deployCarbonCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  #OCMPOSE_FILE_BASE=compose-bft-test-net.yaml
  COMPOSE_FILE_BASE=compose-beefsupply.yaml
  #COMPOSE_BASE_FILES="-f blockchain/compose/${COMPOSE_FILE_BASE} -f blockchain/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
  COMPOSE_BASE_FILES="-f blockchain/compose/docker/${COMPOSE_FILE_BASE}"
  #COMPOSE_COUCH_FILES="-f compose/${COMPOSE_FILE_COUCH} -f blockchain/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}"
  COMPOSE_COUCH_FILES="-f blockchain/compose/docker/${COMPOSE_FILE_COUCH}"
  #COMPOSE_CA_FILES="-f compose/${COMPOSE_FILE_CA} -f blockchain/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CA}"
  COMPOSE_CA_FILES="-f blockchain/compose/docker/${COMPOSE_FILE_CA}"
...
comment
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    #${CONTAINER_CLI} volume rm docker_orderer.beefsupply.com docker_peer0.farmer.beefsupply.com docker_peer0.breeder.beefsupply.com
    ${CONTAINER_CLI} volume rm orderer.beefsupply.com peer0.manager.beefsupply.com peer0.regulator.beefsupply.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/manager/msp organizations/fabric-ca/manager/tls-cert.pem organizations/fabric-ca/manager/ca-cert.pem organizations/fabric-ca/manager/IssuerPublicKey organizations/fabric-ca/manager/IssuerRevocationPublicKey organizations/fabric-ca/manager/fabric-ca-server.db'
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf blockchain/channel-artifacts log.txt *.tar.gz'
  fi
}
...
if [[ $# -ge 1 ]] ; then
  key="$1"
  # check for the createChannel subcommand
  if [[ "$key" == "createChannelCustom" ]]; then
      export MODE="createChannelCustom"
      shift
  # check for the cc command
  elif [[ "$MODE" == "cc" ]]; then
    if [ "$1" != "-h" ]; then
...
if [ "$MODE" == "prereq" ]; then
  ...
  createChannel $BFT
elif [ "$MODE" == "createChannelCustom" ]; then
  infoln "Creating channel '${CHANNEL_NAME}'."
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
  createChannelCustom $BFT
elif [ "$MODE" == "down" ]; then
  ...
  deployCC
elif [ "$MODE" == "deployChaincodeCustom" ]; then
  infoln "deploying chaincode on channel '${CHANNEL_NAME}'"
  deployChaincodeCustom
elif [ "$MODE" == "deployCarbonCC" ]; then
  infoln "deploying chaincode on channel '${CHANNEL_NAME}'"
  deployCarbonCC
