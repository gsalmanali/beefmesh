...
def transmit_model():
	file = open("local_storage/processor_model2.npy", 'rb')

	payload = {'modelname':'processormodel2.npy', 'identity':'http://localhost:5502/'}
	files = {
		'json': ('json_data', json.dumps(payload), 'application/json'),
		'pmodel': ('processormodel2.npy', file, 'application/octet-stream')
	}
...
if __name__ == '__main__':
	app.run(host='localhost', port=5502, debug=False, use_reloader=True)
