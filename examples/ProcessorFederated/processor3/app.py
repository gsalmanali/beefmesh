...
def transmit_condition():
	interface_url = 'http://localhost:5500/usercondition'

	payload = {'user_interface': '5503'}
...
def transmit_model():
	file = open("local_storage/processor_model3.npy", 'rb')

	payload = {'modelname':'processormodel3.npy', 'identity':'http://localhost:5503/'}
	files = {
		'json': ('json_data', json.dumps(payload), 'application/json'),
		'pmodel': ('processormodel3.npy', file, 'application/octet-stream')
	}

	reply = requests.post(url='http://localhost:5504/combinemodel', files=files)
...
if __name__ == '__main__':
	app.run(host='localhost', port=5503, debug=False, use_reloader=True)
