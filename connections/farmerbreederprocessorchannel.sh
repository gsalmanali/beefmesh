...
function createOrgs() {
  ...
  while :
    ...
    infoln "Creating BreederOrderer Org Identities"

    createBreederOrderer

  fi

  infoln "Generating CCP files for Farmer, Breeder and Processor"
...
# call the script to create the channel, join the peers of farmer and breeder,
...
function createChannel() {
  ...
  ./channels/createFarmerBreederProcessorChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}
...
function networkDown() {
  # stop org3 containers also in addition to farmer and breeder, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/consumer/msp organizations/fabric-ca/consumer/tls-cert.pem organizations/fabric-ca/consumer/ca-cert.pem organizations/fabric-ca/consumer/IssuerPublicKey organizations/fabric-ca/consumer/IssuerRevocationPublicKey organizations/fabric-ca/consumer/fabric-ca-server.db'
   docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/regulator/msp organizations/fabric-ca/regulator/tls-cert.pem organizations/fabric-ca/regulator/ca-cert.pem organizations/fabric-ca/regulator/IssuerPublicKey organizations/fabric-ca/regulator/IssuerRevocationPublicKey organizations/fabric-ca/regulator/fabric-ca-server.db'
   docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/manager/msp organizations/fabric-ca/manager/tls-cert.pem organizations/fabric-ca/manager/ca-cert.pem organizations/fabric-ca/manager/IssuerPublicKey organizations/fabric-ca/manager/IssuerRevocationPublicKey organizations/fabric-ca/manager/fabric-ca-server.db'
...
CHANNEL_NAME="farmerbreederprocessorchannel"
...
COMPOSE_FILE_BASE=docker/docker-compose-beefsupply.yaml
