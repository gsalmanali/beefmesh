...
configureadmin() {
	...
	if [ ! -f "$file_path" ]; then
    		...
    		echo "File created: $file_path"
    	        ...
    	        curl -s -S -i --cacert "$SCRIPT_DIR/docker/ssl/certs/ca.crt" -X POST -H "Content-Type: application/json" https://$iot_server_address/tokens -d "{\"email\":\"$user_email\", \"password\":\"$user_pass\"}" > $SCRIPT_DIR/logs/log.txt
	        ...
	        curl -s -S -i --cacert "$SCRIPT_DIR/docker/ssl/certs/ca.crt" -X GET -H "Authorization: Bearer $user_token" https://$iot_server_address/users > $SCRIPT_DIR/logs/log.txt
	        reponse=$(curl -s -S -i --cacert "$SCRIPT_DIR/docker/ssl/certs/ca.crt" -X GET -H "Authorization: Bearer $user_token" https://$iot_server_address/users)
	        file_content=$(<$SCRIPT_DIR/logs/log.txt)
...
newuser() {
	...
	if [ "$#" -ne 1 ]; then
	        ...
	        user_pass=$2
	        SCRIPT_DIR=$(dirname "$(readlink -f "$0")")
	        curl -s -S -i --cacert "$SCRIPT_DIR/docker/ssl/certs/ca.crt" -X POST -H "Content-Type: application/json" https://$iot_server_address/users -d "{\"email\":\"$user_email\", \"password\":\"$user_pass\"}" > $SCRIPT_DIR/logs/log.txt
    ...
    if [ "$#" -ne 1 ]; then
			...
			if [ ! -f "$file_path" ]; then
				echo "user has not signed in yet so no token exist! use signin route!"
	   			...
	   			file_path="$SCRIPT_DIR/env/users/$user_email.sh" 
	   			source $file_path
	   			url_link=https://$iot_server_address/users/$user_id
	   	                # removing \r from end 
	   	                url_link=${url_link%$'\r'} 
	   	                echo $url_link
	   	                echo $user_token
	   	                ...
	   	                curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X GET -H "Authorization: Bearer $user_token" $url_link
    ...
    if [ "$#" -ne 1 ]; then
			...
			if [ ! -f "$file_path" ]; then
				echo "user has not signed in yet so no token exist! use signin route!"
	   			...
	   			file_path="$SCRIPT_DIR/env/users/$user_email.sh" 
	   			source $file_path
	   			url_link=https://$iot_server_address/users
	   	                # removing \r from end 
	   	                #url_link=${url_link%$'\r'} 
	   	                echo $url_link
	   	                echo $user_token
			...
			if [ ! -f "$file_path" ]; then
				echo "user has not signed in yet so no token exist! use signin route!"
	   			...
	   			#url_link=http://localhost/users
	   	                ...
	   	                #echo $user_token
	   	                #echo "Checking token validity now! If its unauthorized, signin to refresh token!"
	   	                curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $user_token" https://$iot_server_address/things/bulk -d '[{"name": "'"$sensor_name"'"}]' > $SCRIPT_DIR/logs/log.txt
	   	                file_content=$(<$SCRIPT_DIR/logs/log.txt)
        	...
        	if [ ! -f "$file_path" ]; then
        		echo "user does not have an ID on record! signup again!"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X GET -H "Authorization: Bearer $user_token" https://$iot_server_address/things > $SCRIPT_DIR/logs/log.txt
        			file_content=$(<$SCRIPT_DIR/logs/log.txt)
        		...
        		echo "user does not have an ID on record! signup again!"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ ! -f "$file_path" ]; then
        				...
        				curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $user_token" https://$iot_server_address/channels/bulk -d '[{"name": "'"$channel_name"'"}]' > $SCRIPT_DIR/logs/log.txt
        	...
        	if [ ! -f "$file_path" ]; then
        		echo "user does not have an ID on record! signup again!"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X GET -H "Authorization: Bearer $user_token" https://$iot_server_address/channels > $SCRIPT_DIR/logs/log.txt
        			file_content=$(<$SCRIPT_DIR/logs/log.txt)
        			echo "Here is the reponse from the platform!"
        			echo $file_content
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
        					...
        					curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt  -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $user_token" https://$iot_server_address/connect -d "$payload" > $SCRIPT_DIR/logs/log.txt
        					file_content=$(<$SCRIPT_DIR/logs/log.txt)
        					echo "Here is the reponse from the platform!"
        					echo $file_content
        				else 
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
        					...
        					#payload="{\"channel_ids\": [\"${channel_id}\"], \"thing_ids\": [\"${sensor_id}\"]}"
        					url_payload=https://$iot_server_address/channels/$channel_id/things/$sensor_id
        					...
        					curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X DELETE -H "Content-Type: application/json" -H "Authorization: Bearer $user_token" $url_link > $SCRIPT_DIR/logs/log.txt
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
    						...
    						if [ -f "$file_path" ]; then
    							...
    							echo $sensor_key
    							url_link=https://$iot_server_address/http/channels/$channel_id/messages
    							# removing \r from end
							url_link=${url_link%$'\r'}
							...
							while IFS=',' read -r row; do
  								...
  								curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X POST -H "Content-Type: application/senml+json" -H "Authorization: Thing $sensor_key" https://$iot_server_address/http/channels/$channel_id/messages -d "$json_payload_escaped"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
    						...
    						source $SCRIPT_DIR/env/keys/$sensor_name.sh
    						echo $sensor_key
      						base_url="https://$iot_server_address:$service_port/channels/$channel_id/messages"
						...
						echo $final_url
    						...
    						curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -H "Authorization: Thing $sensor_key" $final_url
    						#file_content=$(<$SCRIPT_DIR/logs/log.txt)
