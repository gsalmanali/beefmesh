...
configureadmin() {
	...
	if [ ! -f "$file_path" ]; then
	        ...
	        curl -s -S -i --cacert "$SCRIPT_DIR/docker/ssl/certs/ca.crt" -X GET -H "Authorization: Bearer $user_token" http://localhost/users > $SCRIPT_DIR/logs/log.txt
	        reponse=$(curl -s -S -i --cacert "$SCRIPT_DIR/docker/ssl/certs/ca.crt" -X GET -H "Authorization: Bearer $user_token" http://localhost/users)
	        file_content=$(<$SCRIPT_DIR/logs/log.txt)
			...
			if [ ! -f "$file_path" ]; then
				echo "user has not signed in yet so no token exist! use signin route!"
	   			...
	   			#url_link=http://localhost/users
	   	                ...
	   	                #echo $user_token
	   	                #echo "Checking token validity now! If its unauthorized, signin to refresh token!"
	   	                curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $user_token" http://localhost/things/bulk -d '[{"name": "'"$sensor_name"'"}]' > $SCRIPT_DIR/logs/log.txt
	   	                file_content=$(<$SCRIPT_DIR/logs/log.txt)
        	...
        	if [ ! -f "$file_path" ]; then
        		echo "user does not have an ID on record! signup again!"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X GET -H "Authorization: Bearer $user_token" http://localhost/things > $SCRIPT_DIR/logs/log.txt
        			file_content=$(<$SCRIPT_DIR/logs/log.txt)
        		...
        		echo "user does not have an ID on record! signup again!"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ ! -f "$file_path" ]; then
        				...
        				curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $user_token" http://localhost/channels/bulk -d '[{"name": "'"$channel_name"'"}]' > $SCRIPT_DIR/logs/log.txt
        	...
        	if [ ! -f "$file_path" ]; then
        		echo "user does not have an ID on record! signup again!"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X GET -H "Authorization: Bearer $user_token" http://localhost/channels > $SCRIPT_DIR/logs/log.txt
        			file_content=$(<$SCRIPT_DIR/logs/log.txt)
        			echo "Here is the reponse from the platform!"
        			echo $file_content
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
        					...
        					#payload="{\"channel_ids\": [\"${channel_id}\"], \"thing_ids\": [\"${sensor_id}\"]}"
        					url_payload=http://localhost/channels/$channel_id/things/$sensor_id
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
    						...
    						if [ -f "$file_path" ]; then
    							...
    							echo $sensor_key
    							url_link=http://localhost/http/channels/$channel_id/messages
    							# removing \r from end
							url_link=${url_link%$'\r'}
							...
							while IFS=',' read -r row; do
  								...
  								curl -s -S -i --cacert $SCRIPT_DIR/docker/ssl/certs/ca.crt -X POST -H "Content-Type: application/senml+json" -H "Authorization: Thing $sensor_key" http://localhost/http/channels/$channel_id/messages -d "$json_payload_escaped"
        	 	...
        	 	if [ ! -f "$file_path" ]; then
        			...
        			if [ -f "$file_path" ]; then
        				...
        				if [ -f "$file_path" ]; then
    						...
    						source $SCRIPT_DIR/env/keys/$sensor_name.sh
    						echo $sensor_key
      						base_url="http://localhost:$service_port/channels/$channel_id/messages"
