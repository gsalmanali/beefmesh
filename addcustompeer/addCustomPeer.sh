...
#export FABRIC_CFG_PATH=${PWD}/${CUSTOM_PEER_FOLDER_PATH}
#export VERBOSE=false
...
function generateCustomPeer() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    . ${CUSTOM_PEER_FOLDER_PATH}/fabric-ca/registerEnroll.sh
    ...
    test_directory="$SCRIPT_DIR/../organizations/peerOrganizations/$customOrg.beefsupply.com/peers/peer1.$customOrg.beefsupply.com/"
    chmod 777 -R $test_directory
    echo $test_directory
    ls $test_directory
    if [ -d "$test_directory" ]; then
    	echo "$customOrg Peer1 Directory exists."
    	
    	#recreateRegulatorPeer
    else
   	echo "Directory does not exist! Creating new $customOrg Peer1 TLS material"
   	#createRegulatorPeer
    fi
    createCustomPeer
    #createRegulatorPeer
...
function removeCustomPeer(){ 
  ...
  ${CONTAINER_CLI} cp ./${CUSTOM_PEER_FOLDER_PATH}/customscripts cli:/opt/gopath/src/github.com/hyperledger/fabric/peer/customscripts/
 ...
 ./${CUSTOM_PEER_FOLDER_PATH}/customscripts/removeCustomNoCLI.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
 ...
 # if [ $? -ne 0 ]; then
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_CUSTOM_PEER} -f ${COMPOSE_FILE_COUCH_CUSTOM_PEER} down --volumes -remove-orphans
  ...
  ${CONTAINER_CLI} volume stop docker_peer1.$customOrg.beefsupply.com
  ${CONTAINER_CLI} volume rm docker_peer1.$customOrg.beefsupply.com
  #${CONTAINER_CLI} volume rm docker_peer1.regulator.beefsupply.com


  ${CONTAINER_CLI} stop peer1.$customOrg.beefsupply.com
  ${CONTAINER_CLI} rm peer1.$customOrg.beefsupply.com
...
comment
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} -f ${COMPOSE_FILE_CUSTOM_PEER} -f ${COMPOSE_FILE_COUCH_CUSTOM_PEER} up -d 2>&1
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
...
comment
  ...
  ./${CUSTOM_PEER_FOLDER_PATH}/customscripts/updateChannelConfigNoCLI.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi
...
COMPOSE_FILE_COUCH_CUSTOM_PEER=$PWD/${CUSTOM_PEER_FOLDER_PATH}/compose/docker/docker-compose-couch-custom.yaml
# use this as the default docker-compose yaml definition
COMPOSE_FILE_CUSTOM_PEER=$PWD/${CUSTOM_PEER_FOLDER_PATH}/compose/docker/docker-compose-custom.yaml
